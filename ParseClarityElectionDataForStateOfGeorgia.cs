...
namespace ParseClarityElectionDataForStateOfGeorgia
{
	...
	{
		// In Georgia, must omit very many uncontested non-POTUS races.
		// I simplified greatly by just omiting ALL of the US HOUSE races.
	...
	class Program
	{
		...
		static string finalConclusion = "YES, FRAUD: 2% of count of ballots (green line) was added to Joe Biden in most counties.";
		...
		const int ballotsCast = 9999999; // Empirically, from this current input file
		...
		static void inputDetailXmlFile()
		{
			...
			foreach(XmlNode xnContest in xnlContests)
			{
				...
				switch(contest)
				{
						// N.B. These literal strings appear in TWO switches!
					...
					case "US Senate (Perdue)":
						break; // Process this category

						// Too many US House contests were UNCONTESTED in Georgia.
						// Therefore, only sum US SENATE as the "OTHER" to POTUS.
						//	// I revised these manually from inspecting XML, remembering much.
						//case "US House District 1":
						//case "US House District 2":
						//case "US House District 3":
						//case "US House District 4":
						//case "US House District 5":
						//case "US House District 6":
						//case "US House District 8":
						//case "US House District 9":
						//case "US House District 10":
						//case "US House District 11":
						//case "US House District 12":
						//case "US House District 13":
						//case "US House District 14":
						//	continue; // wipppppp...
						//	// break; // Process this category
				...
				foreach(XmlNode xnChoice in xnlChoices)
				{
					...
					// This was the code in Oakland County MI, came from an XML attr:
		...
		//
		// In Oakland County MI, 1.5% was seen.
		//
		// Here, 1.9% was in the StateOfGeorgia data.
		...
		static void LetsRaceTowardTheDesideratum()
		{
			...
			foreach(KeyValuePair<string,Grain>kvp1 in grains)
			{
				...
				// Too many US House contests were UNCONTESTED in Georgia.
				// Therefore, only sum US SENATE as the "OTHER" to POTUS.
				// Hence, as all four[3]==0, omit the divide by two here:
				//int RepOther = four[0].rep + (four[2].rep + four[3].rep) / 2;
				//int DemOther = four[0].dem + (four[2].dem + four[3].dem) / 2;
				int RepOther = four[0].rep + (four[2].rep + four[3].rep);
				int DemOther = four[0].dem + (four[2].dem + four[3].dem);
		...
		static void OutputThePlotResults()
		{
...

			{
				...
				string MeaningLabel = "Republicans are RED, Democrats are BLUE; Lines show % POTUS votes; Areas show % US Senate (Perdue) votes.";
